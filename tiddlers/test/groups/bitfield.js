...
function testBook(expected /*. tiddlerArrays */) {
	...
	var core = utils.testBook(arrays);
	expect(core.state.allVisited()).toEqual(expected);
	return core.state.serialize().output;
};
...
function defaultBitfield() {
	return utils.defaultGroup("set", {"cyoa.serializer": "bitfield", "cyoa.key": "output"});
};
...
it("bridges with multiple connections work",function() {
	...
	var newSerialized = testBook(["1"],tiddlers,extra,
		{title: "Main",text: "<$cyoa touch=2/><$cyoa reset=3/>"});
});
...
it("setting smaller branch sets to root",function() {
	...
	testBook(["A2","Root","X1","X2","X3","X4","X5","X6"],tiddlers.concat(
		{title: "Main",text:"<$cyoa touch='[all[]regexp[^..$]]'/><$cyoa reset=B2/>"}));
});
...
it("can touch and reset with surrounding nodes",function() {
	...
	testBook(["A1","B1","B2","B3","C1","C3","D3"],tiddlers.concat(
		{title: "Main",text:"<$cyoa touch='[all[]regexp[^..$]]'/><$cyoa reset=C2/>"}));
});
...
it("compresses trees to an acceptably small bitfield",function() {
	...
	expect(serialized).toBe("d");
});

it("can clear",function() {
	var defaultGroup = utils.defaultGroup().title;
	testBook([],[node("A"),node("B","A"),node("C"),
		{title: "Main",text: "<$cyoa touch='A B C'/><$cyoa reset='"+defaultGroup+"'/>"}]);
});
...
it("creating tree with irrelevant tiddlers is ignored",function() {
	...
	expect(core.state.allVisited()).toEqual(["C","D"]);
...
it("compresses multiple implications well",function() {
	...
	expect(serialized.length).toBe(1,serialized + " was too long"); // t=!
	testBook(
		["A1","A2","B1","B2","C1","C2","C3","D1","D2","E","F"],
		tiddlers.concat({title: "Main",text: "<$cyoa touch=J/><$cyoa reset=D3/>"}));
	serialized = testBook(
		["A1","B1","C1"],
		tiddlers.concat({title: "Main",text: "<$cyoa touch=J/><$cyoa reset=A2/>"}));
});
...
it("handles multiple implication tricky cases",function() {
	...
	testBook(
		["A1","A2","B","C","D1","E"],
		tiddlers.concat({title: "Main",text:"<$cyoa touch='C E F'/><$cyoa reset=D2/>"}));
});
...
it("compresses exclusions well",function() {
	...
	expect(serialized.length).toBe(1,serialized + " was too long");
	var serialized = testBook(
...
it("separates nodes in different version blocks",function() {
	...
	testBook(["A","X"],tiddlers,extras,{title: "Main",text:"<$cyoa touch='[all[]regexp[^.$]]'/><$cyoa reset=B/>"});
});
...
it("handles implication trees with huge numbers of states",function() {
	...
	var core;
	core = utils.testBook(tree.concat({title: "Main","cyoa.touch": "Y59 Y00"}));
	expect(core.state.allVisited()).toEqual(["X","Y00","Y59"]);
	// Now change things around and try again
	children[0] = "X"; // replace Y00 with X
	children.pop();//remove Y59
	children.push("Z");
	var wiki = new $tw.Wiki();
	core = utils.testBook(tree.concat({title: "Main",text: "<$cyoa touch='[prefix[Y]] Z'/><$cyoa reset='Y00 Y59'/>"}));
	expect(core.state.allVisited()).toEqual(children);
});
